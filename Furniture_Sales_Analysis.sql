/*Furniture Store Sales Data Set*/
...
SELECT SUM(price) AS total_income FROM sales;

/*What is the average daily income?*/
SELECT (SUM(price) / 14) AS average_daily_income FROM sales;

/*What is the total quantity sold and total income generated from each product?*/
SELECT product, COUNT(*) AS quantity_sold, SUM(price) AS total_income FROM sales GROUP BY product ORDER BY quantity_sold DESC;

/*What are the most and least expensive products?*/
SELECT product, MAX(price) AS most_expensive FROM sales;
SELECT product, MIN(price) AS least_expensive FROM sales;

/*What are the 10 biggest sales?*/
SELECT * FROM sales ORDER BY price DESC LIMIT 10;

/*What is the average price of furniture at the store?*/
SELECT AVG(price) AS average_furniture_price FROM sales;

/*What are the payment methods used and how many times were they used?*/
SELECT payment_type, COUNT(*) AS usage FROM sales GROUP BY payment_type ORDER BY usage DESC;

/*Display all transactions made with any payment methods except with Visa and Mastercard.*/
SELECT * FROM sales WHERE payment_type NOT IN ("Visa","Mastercard");

/*From which countries are the sales from and how many sales per country?*/
SELECT country AS list_of_countries, COUNT(*) AS number_of_sales FROM sales GROUP BY country ORDER BY number_of_sales DESC;

/*How many countries have more than one sale?*/
SELECT country AS countries_with_more_than_1_sale, COUNT(*) AS number_of_sales FROM sales GROUP BY country HAVING number_of_sales>1 ORDER BY number_of_sales DESC;

/*Display the 10 biggest sales from the United States.*/
SELECT * FROM sales WHERE country="United states" ORDER BY price DESC LIMIT 10;

/*From which states in the United States are the sales from and how many sales per state?*/
SELECT state AS list_of_states, COUNT(*) AS number_of_sales FROM sales WHERE country="United states" GROUP BY state ORDER BY number_of_sales DESC;

/*How many states have more than 1 sale?*/
SELECT COUNT(*) AS number_of_sales,
    ...
    END AS "states_with_more_than_1_sale"
FROM sales GROUP BY states_with_more_than_1_sale ORDER BY number_of_sales ASC LIMIT 11;

/*From which continents are the sales from and what is the percentage of sales per continent?*/
SELECT COUNT(*) AS percentage_of_sales,
    CASE
    WHEN (latitude>=-30.0000 AND longitude<=140.0000) THEN "Oceania"
    WHEN (latitude>=54.525961 AND longitude<=15.255119) THEN "Europe"
    WHEN (latitude>=-82.86275189999999 AND longitude<=135) THEN "Antarctica"
    WHEN (latitude>=-8.783195 AND longitude<=-55.491477) THEN "South America"
    WHEN (latitude>=54.525961 AND longitude<=-105.255119) THEN "North America"
    WHEN (latitude>=-8.783195 AND longitude<=34.50852299999997) THEN "Africa"
    WHEN (latitude>=34.047863 AND longitude<=100.619655) THEN "Asia"
    END AS "continents"
FROM sales GROUP BY continents ORDER BY percentage_of_sales ASC LIMIT 7;
