...
-- Select all the directors and their projects.
SELECT people.director, titles.title, titles.type
FROM "netflix_people" people LEFT JOIN "netflix_titles_info" titles
ON people.show_id = titles.show_id;

-- How many movie titles are there in the database?
SELECT COUNT(*) AS "Number_Of_Movies" FROM "netflix_titles_info"
WHERE type='Movie';

-- When was the most recent batch of tv shows and/or movies added to the database?
SELECT MAX(DATE(date_added)) AS "Date" FROM "netflix_titles_info";
...
SELECT director
FROM "netflix_titles_info" titles LEFT JOIN "netflix_people" people
ON titles.show_id = people.show_id WHERE titles.title='The Starling';

-- What is the oldest movie in the database and what year was it made?
SELECT title, release_year FROM "netflix_titles_info"
WHERE type='Movie' ORDER BY release_year ASC LIMIT 1;

-- What is/are the newest movie/movies in the database and what year was/were it/they made?
SELECT title, release_year FROM "netflix_titles_info"
WHERE type = 'Movie' AND release_year >=
(SELECT MAX(release_year) FROM "netflix_titles_info" WHERE type = 'Movie');
